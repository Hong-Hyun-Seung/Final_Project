...
class DenseRetrieval:
    def __init__(
        ...
        self.train_datadict, self.contexts, self.places = get_train_dataset(
    ...
    def inference(self, single_query, topk=5):
        if self.p_embedding == None:
            self.get_embedding()
        doc_scores, doc_indices = self.get_relevant_doc(single_query, k=topk)
        total = []
        for i in range(topk):
            ...
            if epoch >= args.val_start_epoch:
                with torch.no_grad():
                    ...
                    for datum in tqdm(val_data):
                        ...
                        q_emb = q_encoder(**q_seqs_val).pooler_output.to(
                            "cpu"
                        )  # (num_query, emb_dim) 
                        #query(valdata[쿼리])에 대한 cls token이 q_emb에 담긴다
                        #p_embs는 passage들에 대한 cls토큰들이 저장이 돼 있다
                        dot_prod_scores = torch.matmul(
                            q_emb, torch.transpose(p_embs, 0, 1)
                        )
                        scores = dot_prod_scores.numpy()
                        tmp = []
                        for i, v in enumerate(scores[0]): #score[0]의 역할은 괄호 벗기기
                            info = {
                                "place": self.places[i],
                                "score": v,
                            }
                            tmp.append(info) #tmp에 각 지역마다 지역이름, dot product score가 담기게 된다
                        rank_ = sorted(tmp, key=lambda x: -x["score"]) #tmp를 dot product score를 기준으로 sorting한다 -> rank_
                        check = False
                        for i, idx in enumerate(rank_[:100]):#dot product score top 100지역 
                            if sample_place == idx["place"]: #sample_place(valdata[지역])이 3줄 위의 rank_ 안에 있으면 
                                for k in top: #top = {1: 0, 5: 0, 10: 0, 30: 0, 50: 0, 100: 0}
                                    if i < k: # if i < 1 => top[1]을 하나 추가 한다 그럼, {1: 1, 5: 0, 10: 0, 30: 0, 50: 0, 100: 0} 이 되겠지
                                        top[k] += 1
                                        if k == 100:
                                            check = True
                                if check:
                ...
                for k in top:
                    ...
                    wandb.log({f"val/top-{k} acc": top[k]}, step=global_step)
                for k in top:
                    if top[k] > best_top[k]:
                        best_top[k] = top[k]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_path",
        default="/opt/ml/final-project-level3-nlp-11/data/MODEL",
        type=str,
        help="",
    )
    parser.add_argument(
